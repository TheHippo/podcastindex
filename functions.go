...
func (c *Client) Search(term string) ([]*Podcast, error) {
	return c.SearchC(term, false, false, 0)
}
...
func (c *Client) SearchC(term string, clean bool, fullText bool, max uint) ([]*Podcast, error) {
	url := fmt.Sprintf("search/byterm?q=\"%s\"%s%s%s", term, addClean(clean), addFullText(fullText), addMax(max))
...
func (c *Client) PodcastByFeedURL(url string) (*Podcast, error) {
	u := fmt.Sprintf("podcasts/byfeedurl?url=%s", url)
...
func (c *Client) PodcastByFeedID(id uint) (*Podcast, error) {
	url := fmt.Sprintf("podcasts/byfeedid?id=%d", id)
...
func (c *Client) PodcastByITunesID(id uint) (*Podcast, error) {
	url := fmt.Sprintf("podcasts/byitunesid?id=%d", id)
